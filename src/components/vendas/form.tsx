...
export const VendasForm: React.FC<VendasFormProps> = ({
    ...
    const handleAddProduto = () => {
        const itensAdicionados = formik.values.items
    ...
    const totalVenda = () =>{
        const totais : number[] = formik.values.items?.map(iv => iv.quantidade * iv.produto.preco);
        if(totais.length){
            return totais.reduce((sumatoria = 0, valorItemActual) => sumatoria + valorItemActual);
        }else {
            return 0;
        }
    } 
            ...
            <div className="p-fluid">
                ...
                <div className="p-grid">
                    ...
                    <div className="p-col-12">
                        <DataTable value={formik.values.items} emptyMessage="Ningun producto agregado.">
                            <Column body ={ (item: ItemVenda) => {
                                const handleRemoverItem = () => {
                                    const novaLista = formik.values.items.filter(
                                        iv => iv.produto.id != item.produto.id
                                    )
                                    formik.setFieldValue("items", novaLista)
                                }
                                return (
                                    <Button type="button" label="Quitar" onClick={handleRemoverItem}/>
                                )
                            }}
                            />
                    ...
                    <div className='p-col-5'>
                        <div className='p-field'>
                            <label htmlFor='formaPagamento'>Forma de pago: *</label>
